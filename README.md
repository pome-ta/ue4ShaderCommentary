...
1. 時間経過とともに、`Time` から吐き出される数字が大きくなる
1. `Sine` 関数に入ると、必ず`-1.0 ~ 1.0` の数値が順に出てくる
...
当然のごとく、なにかの数値を変化させれば、なにかが変わるわけです
...
- `Sine`
  ...
  - 最終結果のMax値が、`1.0` だとすると、`Abs` に渡す前の演算だと、ピッタリ行かない気がする。。。
...
これで、カウントアップされる数値が1/10 遅くなるです。

なぜ、割り算(`Divide`) ではなく掛け算(`Multiply`) なのか？

`0` を割ろうとすると、エラーになったりして面倒なので！(私の脳死プレイ)開始時には、0秒というのが有り得そうだと、キモいので、それならば掛け算したほうが楽じゃね？って浅はかな考えです



速度を上げたい場合には、1以上の数値を掛けてあげればいいです



### 発色する色を変える

やっと、後述のフラグが回収できます！やったね！
...
いままでは、赤とか、青とか好きな色を出していたのに！と


この理由がわかると、`Debug` 系のノードを使えるようになるので、検証もしやすいし、実験もしやすいし、失敗している理由もわかりやすくなります



#### 白だった理由

`final_color` に、`(0.0, 0.0, 0.0)~ (1.0, 1.0, 1.0)` の数値をいれていたからです

なんで、一つのピンで挿入しているのに、`RGB` の3つの値が入るのか？

`Constant3Vector` の仕組みがわかると理解できます

`Constant3Vector` は、`RGB` の値を事前に指定をしました

この値を `vec3` と言います。`vec3` として、3つの数値の情報をもった塊を一つの値とするのです

ちなみに、一つの値の場合は、`Scalar` とか`float` とか呼ばれます


`out` が`float` の生身の1つの値で、`in` が`vec3` で、1つのピンだとしても実行はできます。1つの値を、`vec3` のひとつひとつに、ぶち込むからです



つまり、今回の点滅であれば`final_color` へ、`R`, `G`, `B` の`vec3` 情報を送りつければ色が変更できるわけです
...
変化しない場合などは、そうやって、不要な数値を入れていて隠蔽されている可能性があります



アルファ値でいうと、`Transparency` か`Opacity` とか、ピンがあったはず
...
我々は、`RGB` それぞれの数値を自由にいじり、合体させて`final_color` に送ることができるようになりました

つまり、点滅する3種それぞれに、違う関数を入れると変化を楽しめるようになったということです
...
掛け算の数値をそれぞれバラバラにしてあげると、色々な変化が見られると思います
